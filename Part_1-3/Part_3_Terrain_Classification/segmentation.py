...
# HuggingFace fallback when local model directory does not exist
_HF_LOVEDA_MODEL = "wu-pr-gw/segformer-b2-finetuned-with-LoveDA"

# ---------------------------------------------------------------------------
# GAMUS semantic class reference
# ---------------------------------------------------------------------------
# GAMUS (Geometry-Aware Multi-modal Semantic Segmentation Benchmark) provides
# RGB + nDSM + semantic labels for remote-sensing imagery.
# Official classes (used for future GAMUS-based fine-tuning/validation):
#
#   0  other / background
#   1  ground
#   2  low vegetation
...
#   6  tree
#
# Our current 5-class TerrainClass mapping (compatibility layer):
#   OTHER      = 0   ← GAMUS 0 (other) + 1 (ground)
#   VEGETATION = 1   ← GAMUS 2 (low vegetation) + 6 (tree) [merged]
#   BUILDING   = 2   ← GAMUS 3 (building)
#   ROAD       = 3   ← GAMUS 5 (road)
#   WATER      = 4   ← GAMUS 4 (water)
#
# NOTE: GAMUS nDSM is height-above-ground (metres), NOT absolute elevation.
#       The Part 2 DSM is absolute elevation above SRTM datum.
#       These two quantities must never be directly compared.
#
# GAMUS inference status: No local GAMUS checkpoint loaded.
# The GAMUS metric framework (MAE, Spearman-r, delta-accuracy) is used
# for structural consistency evaluation in run_pipeline.py.
# ---------------------------------------------------------------------------
_GAMUS_TO_TERRAINCLASS = {
    0: 0,  # other/background → OTHER
    1: 0,  # ground           → OTHER
    2: 1,  # low vegetation   → VEGETATION
    3: 2,  # building         → BUILDING
    4: 4,  # water            → WATER
    5: 3,  # road             → ROAD
    6: 1,  # tree             → VEGETATION (collapsed; tree not separately tracked yet)
}


def _resolve_model_path(model_name: str, hf_fallback: str) -> str:
    """Return model_name if it exists locally, otherwise the HF fallback ID.

    This lets the pipeline work whether the model is saved locally at
    ``model_name`` (a relative or absolute path) or only present in the
...
def apply_building_cleanup(
    label_map: np.ndarray,
    min_building_area_px: int = 100,
    close_radius: int = 3,
    open_radius: int = 2,
) -> np.ndarray:
    """Morphological cleanup of the building class to reduce noise without erasing real buildings.

    Operations (in order)
    ---------------------
    1. Morphological closing (dilation then erosion): fills small holes inside
       building footprints caused by skylights, shadows, or tile-boundary gaps.
       Uses a disk of radius ``close_radius`` pixels.
    2. Morphological opening (erosion then dilation): removes isolated single-pixel
       and very small building speckles that survive the model + fallbacks.
       Uses a disk of radius ``open_radius`` pixels.
    3. Connected-component size filter: remove any remaining blobs smaller than
       ``min_building_area_px`` pixels.  This is the same threshold used by
       ``_extract_building_instances`` so the cleanup and instance extraction
       are consistent.

    The result is a *new* label map — the input is not mutated.
    Non-building classes are preserved exactly.

    Parameters
    ----------
    label_map            : 2-D uint8 array (values 0–4)
    min_building_area_px : minimum connected-component area to keep (pixels).
    close_radius         : dilation radius for closing step (pixels).
    open_radius          : erosion radius for opening step (pixels).

    Returns
    -------
    np.ndarray — 2-D uint8 array with the same shape as ``label_map``.
    """
    ...
    result = label_map.copy()
    bldg_mask = (label_map == int(TerrainClass.BUILDING))

    if not bldg_mask.any():
        return result

    # --- Step 1: close small holes inside building footprints ---------------
    disk_close = _disk_struct_seg(close_radius)
    closed = binary_closing(bldg_mask, structure=disk_close)

    # Only ADD pixels that were OTHER (0) — never steal from other classes
    other_mask  = (label_map == int(TerrainClass.OTHER))
    new_bldg_from_close = closed & (~bldg_mask) & other_mask
    bldg_after_close    = bldg_mask | new_bldg_from_close

    # --- Step 2: open to remove tiny speckles -------------------------------
    disk_open = _disk_struct_seg(open_radius)
    opened = binary_opening(bldg_after_close, structure=disk_open)

    # Pixels removed by opening that were OTHER → keep as OTHER (no change)
    # Pixels removed by opening that were BUILDING → revert to OTHER
    removed_by_open = bldg_after_close & (~opened)
    result[removed_by_open] = int(TerrainClass.OTHER)
    result[opened & (~bldg_mask)] = int(TerrainClass.BUILDING)   # newly added by close
    result[opened & bldg_mask]    = int(TerrainClass.BUILDING)   # keep existing buildings

    # --- Step 3: size filter ------------------------------------------------
    bldg_final = (result == int(TerrainClass.BUILDING))
    if not bldg_final.any():
        return result

    labeled, n_feats = cc_label(bldg_final)
    if n_feats == 0:
        return result

    blob_ids = np.arange(1, n_feats + 1)
    areas    = ndi_sum(bldg_final, labeled, blob_ids)
    slices   = find_objects(labeled)

    for i, sl in enumerate(slices):
        if sl is None:
            continue
        if int(areas[i]) < min_building_area_px:
            blob_id = i + 1
            # Use bounding-box slice — avoids a full H×W scan per tiny blob
            local_labeled = labeled[sl[0], sl[1]]
            local_result  = result[sl[0], sl[1]]
            local_result[local_labeled == blob_id] = int(TerrainClass.OTHER)

    return result


def _disk_struct_seg(radius: int) -> np.ndarray:
    """Boolean disk structuring element for segmentation morphological ops."""
    y, x = np.ogrid[-radius:radius + 1, -radius:radius + 1]
    return (x ** 2 + y ** 2) <= radius ** 2


def apply_water_validation(
    label_map: np.ndarray,
    logits_full: Any,
    min_water_prob: float = 0.35,
    min_water_area_px: int = 500,
    water_channel: int = 4,
) -> np.ndarray:
    """Suppress water false positives using model confidence + spatial coherence.

    Strategy
    --------
    Water pixels are accepted only when ALL of:
      1. Model softmax probability for WATER channel ≥ min_water_prob, AND
      2. The connected component containing the pixel has area ≥ min_water_area_px.

    This rejects small isolated blue-ish patches (blue roofs, glass, shadows,
    painted surfaces) while preserving large coherent water bodies (rivers,
    lakes, flooded areas, reservoirs).

    Parameters
    ----------
    label_map          : 2-D uint8 label array (values 0–4).
    logits_full        : (1, C, H, W) tensor — full-resolution logits from SegFormer.
    min_water_prob     : minimum per-pixel water softmax probability (default 0.35).
    min_water_area_px  : minimum connected component area in pixels (default 500).
    water_channel      : model output channel index for water (default 4).

    Returns
    -------
    np.ndarray — copy of label_map with unreliable water pixels reverted to OTHER.
    """
    import torch
    from scipy.ndimage import label as cc_label, sum as ndi_sum, find_objects

    result = label_map.copy()
    water_mask = (label_map == int(TerrainClass.WATER))
    if not water_mask.any():
        return result

    # --- Confidence gate: revert low-probability water pixels ---------------
    with torch.no_grad():
        probs = torch.nn.functional.softmax(logits_full[0], dim=0)
    water_prob = probs[water_channel].cpu().numpy()  # (H, W)
    del probs

    low_conf = water_mask & (water_prob < min_water_prob)
    result[low_conf] = int(TerrainClass.OTHER)

    # Recompute water mask after confidence filter
    water_mask = (result == int(TerrainClass.WATER))
    if not water_mask.any():
        return result

    # --- Spatial coherence: remove tiny isolated water blobs ----------------
    labeled, n = cc_label(water_mask)
    if n == 0:
        return result
    blob_ids = np.arange(1, n + 1)
    areas    = ndi_sum(water_mask, labeled, blob_ids)
    slices   = find_objects(labeled)

    for i, sl in enumerate(slices):
        if sl is None:
            continue
        if int(areas[i]) < min_water_area_px:
            blob_id = i + 1
            local_lab = labeled[sl[0], sl[1]]
            result[sl[0], sl[1]][local_lab == blob_id] = int(TerrainClass.OTHER)

    return result


def apply_road_refinement(
    label_map: np.ndarray,
    logits_full: Any = None,
    road_prob_floor: float = 0.15,
) -> np.ndarray:
    """Improve road spatial continuity using model probability + morphological closing.

    Two-stage approach:
    1. If logits_full is provided: recover road pixels where model probability
       ≥ road_prob_floor but they were classified as OTHER due to close competition
       with other classes. These are near-road pixels the model was uncertain about.
    2. Morphological closing in horizontal and vertical directions fills short gaps
       in the road network (tile boundaries, narrow occlusions).

    Parameters
    ----------
    label_map     : 2-D uint8 label array.
    logits_full   : optional (1, C, H, W) tensor — enables probability-gated recovery.
    road_prob_floor : minimum model road probability to recover a pixel (default 0.15).

    Returns
    -------
    np.ndarray — copy with improved road continuity.
    """
    from scipy.ndimage import binary_closing, binary_dilation

    result    = label_map.copy()
    road_mask = (label_map == int(TerrainClass.ROAD))

    # --- Stage 1: probability-gated road recovery -------------------------
    if logits_full is not None:
        try:
            import torch
            road_channel = int(TerrainClass.ROAD)
            with torch.no_grad():
                probs = torch.nn.functional.softmax(logits_full[0], dim=0)
            road_prob = probs[road_channel].cpu().numpy()
            del probs
            # Recover OTHER pixels where road probability ≥ floor
            # Only where road already exists nearby (dilation guard prevents
            # spurious road pixels far from any existing road region)
            if road_mask.any():
                road_neighborhood = binary_dilation(
                    road_mask, structure=np.ones((15, 15), dtype=bool)
                )
                recoverable = (
                    (result == int(TerrainClass.OTHER))
                    & (road_prob >= road_prob_floor)
                    & road_neighborhood
                )
                result[recoverable] = int(TerrainClass.ROAD)
                road_mask = (result == int(TerrainClass.ROAD))
        except Exception:
            pass
    ...
    # --- Stage 2: morphological closing to bridge short gaps --------------
    # Use slightly wider kernels than the old 1×9 — roads at 0.3m/px can
    # be 10–30px wide; a 15px horizontal/vertical close bridges tile gaps.
    closed_h = binary_closing(road_mask, structure=np.ones((1, 15), dtype=bool))
    closed_v = binary_closing(road_mask, structure=np.ones((15, 1), dtype=bool))
    combined = closed_h | closed_v

    # Only add new road pixels where current label is OTHER
    new_road = combined & (~road_mask) & (result == int(TerrainClass.OTHER))
    result[new_road] = int(TerrainClass.ROAD)
    return result


def _watershed_separate_buildings(
    building_mask: np.ndarray,
    dsm_crop: np.ndarray | None = None,
) -> np.ndarray:
    """Separate touching buildings using distance-transform watershed.

    Uses the Euclidean distance transform of the binary building mask to find
    local maxima (building cores) and a marker-based watershed to draw
    boundaries between adjacent buildings.

    If `dsm_crop` is provided (same shape as building_mask), height
    discontinuities are used to reinforce boundaries by inverting the local
    DSM gradient to add separation cues.

    Returns
    -------
    np.ndarray int32 — labeled array (0 = background, ≥1 = building instances).
    """
    try:
        ...
        # Fallback: plain connected components without separation
        from scipy.ndimage import label as cc_label
        labeled, _ = cc_label(building_mask)
        return labeled.astype(np.int32)

    if not building_mask.any():
        return np.zeros_like(building_mask, dtype=np.int32)

    # Distance transform: each foreground pixel's distance to nearest background
    dist = distance_transform_edt(building_mask).astype(np.float32)

    # Optionally reinforce with DSM height discontinuity
    if dsm_crop is not None and dsm_crop.shape == building_mask.shape:
        valid = np.isfinite(dsm_crop) & building_mask.astype(bool)
        if valid.sum() > 100:
            # Normalise DSM to [0, 1] within the building region
            lo = float(dsm_crop[valid].min())
            hi = float(dsm_crop[valid].max())
            if hi > lo + 0.5:
                dsm_norm = np.clip((dsm_crop - lo) / (hi - lo), 0, 1).astype(np.float32)
                dsm_norm[~building_mask.astype(bool)] = 0
                # Height adds to distance signal: higher peaks → stronger seeds
                dist = dist + 0.5 * dsm_norm * dist.max()

    # Suppress distance transform outside mask
    dist[~building_mask.astype(bool)] = 0

    # Local maxima as watershed seeds — use a footprint that prevents seeds
    # from two adjacent buildings merging: 15×15 is reasonable at 5000×5000 px
    footprint = np.ones((15, 15), dtype=bool)
    local_max = (maximum_filter(dist, footprint=footprint) == dist) & (dist > 2.0)

    # Label seeds
    markers, n_markers = cc_label(local_max)
    if n_markers == 0:
        # No seeds found — fall back to single-component
        markers[building_mask.astype(bool)] = 1
        n_markers = 1

    # Watershed on negated distance (flooding from peaks)
    watershed_input = (dist.max() - dist).astype(np.uint8)
    try:
        labels_ws = cv2.watershed(
            cv2.cvtColor(watershed_input[:, :, np.newaxis].repeat(3, axis=2), cv2.COLOR_GRAY2BGR),
            markers.astype(np.int32),
        )
    except Exception:
        labels_ws = markers.astype(np.int32)

    # Mask out background
    labels_ws = labels_ws.astype(np.int32)
    labels_ws[~building_mask.astype(bool)] = 0
    labels_ws[labels_ws < 0] = 0  # watershed boundary markers → background
    return labels_ws
...
def segment(
    ...
    *,
    ...
    resolved_model = _resolve_model_path(str(model_name), _HF_LOVEDA_MODEL)
    processor, model, torch, resolved_device = _load_model(resolved_model, device)
...
_DETECTION_SCHEMA: dict = {
    ...
    "properties": {
        ...
        "mean_building_probability": {"type": "number", "minimum": 0.0, "maximum": 1.0},
    },
}
...
def _extract_building_instances(
    ...
    dsm_array: np.ndarray | None = None,
) -> list[dict]:
    ...
    # For a 5000×5000 dense urban scene, watershed over the full image can
    # massively over-segment.  Instead we use a lightweight erosion-based
    # separation: erode the building mask slightly to break thin connections
    # between adjacent buildings, then label connected components, then map
    # labels back to the original (un-eroded) mask.
    # This handles the common case of two buildings separated by a 2–5 px gap
    # in the semantic mask without over-splitting large flat-roof structures.
    # DSM height discontinuity can optionally reinforce the erosion boundaries
    # when available.

    # Step 1: Optional DSM-guided separation — find height-discontinuity boundary
    separation_mask = np.ones_like(building_mask, dtype=bool)  # True = keep
    if dsm_array is not None:
        try:
            # Compute gradient magnitude of DSM within building footprint.
            # Use Scharr operator (more sensitive than Sobel for small gradients).
            import cv2 as _cv2
            dsm_f = dsm_array.astype(np.float32)
            dsm_f[building_mask == 0] = np.nan
            # Fill NaN with interpolated values for gradient computation
            dsm_filled = dsm_f.copy()
            nan_mask = np.isnan(dsm_filled)
            if nan_mask.any():
                dsm_filled[nan_mask] = float(np.nanmedian(dsm_f))
            grad_x = _cv2.Scharr(dsm_filled, _cv2.CV_32F, 1, 0)
            grad_y = _cv2.Scharr(dsm_filled, _cv2.CV_32F, 0, 1)
            grad_mag = np.sqrt(grad_x**2 + grad_y**2)
            grad_mag[building_mask == 0] = 0.0
            # Use P75 of building-interior gradients as threshold — adaptive
            # to actual DSM variance rather than a fixed 0.5m cutoff.
            bldg_grads = grad_mag[building_mask == 1]
            if len(bldg_grads) > 100:
                threshold = float(np.percentile(bldg_grads, 75))
                if threshold > 0.1:  # any meaningful gradient
                    separation_mask = (grad_mag < threshold) | (building_mask == 0)
        except Exception:
            pass  # DSM guidance optional — proceed without it

    # Step 2: Erode mask with a small structuring element to break thin joints
    # 3×3 erosion: separates buildings with <3 px gap in the semantic mask
    eroded = binary_erosion(building_mask, structure=np.ones((3, 3), dtype=bool))
    if dsm_array is not None:
        eroded = eroded & separation_mask

    # Step 3: Label the eroded mask (separated building seeds)
    labeled_eroded, num_seeds = cc_label(eroded)

    # Step 4: Assign each original building pixel to its nearest seed.
    # Use scipy distance_transform_edt indices — this is O(N) not O(N*iterations)
    # and produces correct Voronoi-style assignment without a slow loop.
    labeled_array = np.zeros_like(building_mask, dtype=np.int32)
    if num_seeds > 0:
        try:
            from scipy.ndimage import distance_transform_edt as _dte
            # For each background pixel of labeled_eroded, find nearest seed index
            # edt with return_indices gives the (row,col) of the nearest non-zero
            _, nearest_idx = _dte(labeled_eroded == 0, return_indices=True)
            # Map each building pixel to the nearest seed label
            r_idx = nearest_idx[0].ravel()
            c_idx = nearest_idx[1].ravel()
            propagated = labeled_eroded[r_idx, c_idx].reshape(building_mask.shape)
            # Only assign labels inside the original (un-eroded) building mask
            labeled_array = propagated * building_mask
            num_features = int(labeled_array.max())
        except Exception:
            # Fallback: slower iterative propagation
            from scipy.ndimage import maximum_filter as _maxfilt
            current = labeled_eroded.copy()
            struct = np.ones((5, 5), dtype=bool)
            for _ in range(25):
                dilated_step = _maxfilt(current, footprint=struct)
                dilated_step = dilated_step.astype(np.int32) * building_mask
                if (dilated_step == current).all():
                    break
                current = dilated_step
            labeled_array = current
            num_features = int(labeled_array.max())
    else:
        # Fallback: plain CC on the full building mask
        labeled_dilated, num_features = cc_label(
            binary_dilation(building_mask, structure=np.ones((5, 5), dtype=bool))
        )
        labeled_array = labeled_dilated * building_mask
        num_features = int(labeled_array.max())
    ...
    for i, sl in enumerate(blob_slices):
        ...
        # Mean building probability over the blob — this is mean segmentation
        # softmax probability, NOT an object-detection confidence score.
        mean_building_prob = float(round(float(building_prob[sl[0], sl[1]][local_mask == 1].mean()), 4))
        ...
        instances.append({
            ...
            "mean_building_probability": mean_building_prob,
            # Keep "confidence" as an alias for backward compatibility with
            # downstream consumers that expect this key.
            "confidence":               mean_building_prob,
        })
...
def classify_terrain(
    ...
    *,
    ...
    # --- load LoveDA model (resolve local path → HF fallback if needed) ------
    resolved_model = _resolve_model_path(str(model_name), _HF_LOVEDA_MODEL)
    processor, model, torch_mod, resolved_device = _load_model(resolved_model, device)
    ...
    # NOTE: apply_warmth_fallback and apply_brightness_fallback are DISABLED.
    # Colour-based building promotion (red/bright = building) causes severe
    # false positives on natural terrain (bare rock, mountains, desert, brick
    # roads, shadows) — critical failure modes for disaster/hilly-terrain use.
    # Only the greenness fallback is retained; it has strong confidence gating
    # so it only fires on genuinely model-uncertain pixels.
    if enable_fallbacks:
        ...
        # apply_warmth_fallback DISABLED — causes false building detections on
        # rocky/mountainous terrain, arid land, terracotta roads, red soil.
        # apply_brightness_fallback DISABLED — causes false building detections
        # on snow, sand, concrete roads, bright natural surfaces.

    # --- water validation: suppress false positives -----------------------
    # Model water predictions are validated for spatial coherence and minimum
    # area before being accepted.  This suppresses blue-roof, blue-glass, blue
    # shadow, and blue-painted-surface false positives while preserving genuine
    # spatially-coherent water bodies (rivers, lakes, reservoirs, floods).
    label_map = apply_water_validation(
        label_map,
        logits_full=logits_full,
        min_water_prob=0.35,        # pixel must have ≥35% water probability
        min_water_area_px=500,      # ignore tiny isolated blobs (<500 px)
        water_channel=int(TerrainClass.WATER),
    )

    # --- road spatial refinement ------------------------------------------
    label_map = apply_road_refinement(
        label_map,
        logits_full=logits_full,
        road_prob_floor=0.12,
    )

    t_post = _time.perf_counter()

    # --- optional DSM loading for geometry-aware separation ---------------
    _dsm_array: np.ndarray | None = None
    if dsm_path is not None:
        try:
            import rasterio as _rio
            with _rio.open(dsm_path) as _ds:
                _dsm_array = _ds.read(1).astype(np.float32)
            if _dsm_array.shape != (H, W):
                _dsm_array = None  # shape mismatch — skip DSM guidance
        except Exception:
            _dsm_array = None

    # --- building morphological cleanup (speckle removal + hole filling) ----
    label_map = apply_building_cleanup(
        label_map,
        min_building_area_px=_MIN_BUILDING_AREA_PX,
        close_radius=3,
        open_radius=2,
    )
    ...
    instances = _extract_building_instances(
        label_map, logits_full, id2label=id2label, dsm_array=_dsm_array
    )
    ...
    n_bldg_inst = len([i for i in instances if i['class'] == 'building'])
    print(f"  Output: {W}×{H}px  building_instances={n_bldg_inst}  "
    ...
    # Report raw water probability range for diagnostics (helps identify false-positive source)
    water_ch = int(TerrainClass.WATER)
    if logits_full is not None:
        with torch_mod.no_grad():
            water_prob_arr = torch_mod.nn.functional.softmax(logits_full[0], dim=0)[water_ch].cpu().numpy()
        w_max  = float(water_prob_arr.max())
        w_mean = float(water_prob_arr.mean())
        w_gt35 = float((water_prob_arr > 0.35).mean() * 100)
        print(
            f"  [diag] water model prob: max={w_max:.4f}  mean={w_mean:.4f}  "
            f">35%: {w_gt35:.2f}% of pixels",
            flush=True,
        )
        if w_max < 0.05:
            print("  [diag] WARNING: water channel max < 0.05 — model has no water signal. "
                  "Domain shift likely. Any water predictions are unreliable.", flush=True)
        del water_prob_arr
...
def visualize_confidence_heatmap(
    ...
    *,
    ...
    processor, model, torch_mod, resolved_device = _load_model(
        _resolve_model_path(str(model_name), _HF_LOVEDA_MODEL), device
    )
    id2label = model.config.id2label
